...
network nodes.
Sequential single-machine simulation is the universal
methodology in algorithmic federated learning research,
not a limitation specific to this work.
The three foundational algorithms against which this
thesis is benchmarked were each developed and validated
exclusively through simulated client processes on a
single machine: McMahan~et~al.\ executed all FedAvg
convergence experiments by simulating up to 100 clients
sequentially on a single server \cite{mcmahan2017fedavg};
Karimireddy~et~al.\ validated SCAFFOLD on synthetic
non-IID partitions in a centralised simulation
\cite{karimireddy2020scaffold}; and Li~et~al.\
evaluated FedProx under simulated system heterogeneity
on a single machine \cite{li2020fedprox}.
Single-machine simulation isolates algorithmic
convergence behaviour from network engineering
variables such as latency, bandwidth, and dropout,
which are orthogonal to the research questions
addressed here.
As stated in the architecture description
(Section~\ref{sec:client-simulation}), sequential
execution introduces no algorithmic difference
relative to a true parallel deployment: each client
receives an identical copy of the global model,
trains independently on its local partition, and
returns its parameter vector before the next client
begins \cite{beutel2022flower}.
...
\begin{table}[htbp]
  ...
  \caption{Binary data partition across three simulated
    hospital clients applied to the fold~0 training set
    (590 patients, seed=42, $\alpha=0.5$).
    The 147 fold~0 validation patients are held out as
    the shared evaluation set for both centralised and
    federated experiments.
    Hospital~3 is the only client with a Non-Luminal
    majority, generated by Dirichlet $\alpha=0.5$,
    seed=42.}
  ...
  \begin{tabular}{lcccc}
    ...
    \textbf{Client} & \textbf{Luminal}
      ...
      & \textbf{Local ratio} \\
    \midrule
    Hospital 1 & 354 & 58  & 412 & 6.1:1 \\
    Hospital 2 & 63  & 44  & 107 & 1.4:1 \\
    Hospital 3 & 32  & 39  &  71 & 0.8:1 \\
    \midrule
...
The federated experiment comprises four runs:
(1)~FedAvg, $\alpha=0.5$ (primary baseline);
(2)~SCAFFOLD, $\alpha=0.5$ (primary contribution);
(3)~FedAvg, $\alpha=100$ (near-IID comparison);
(4)~SCAFFOLD, $\alpha=100$ (near-IID comparison).
The $\alpha=0.1$ configuration is excluded because
it produces a degenerate partition under seed=42
(Hospital~1: 734 patients, Hospital~2: 1,
Hospital~3: 2).
Results for runs~(1) and~(2) are the primary
contribution; runs~(3) and~(4) isolate the effect
of heterogeneity degree on the SCAFFOLD gain.
...
\begin{table}[htbp]
  \centering
  \caption{Binary federated learning results for all six
    experimental runs ($R=20$ rounds, $E=3$ local epochs,
    $K=3$ hospitals). Binary centralised fold~0 result
    shown as the performance ceiling.}
...
quantify the federated performance gap and the SCAFFOLD
